...
def expert_high_level_policy(cup_feature: dict, config: dict) -> str:
    ...
    release_count = int(cup_feature.get("release_count", 0))
    ...
    if user_present == 1 and not used_cup_candidate:
        if stationary_time >= never_active_ask_delay:
            return "ASK"
        return "IDLE"

    if used_cup_candidate and user_present == 1:
        if (
            release_count > 0
            and hand_distance >= touch_threshold
            and time_since_release >= ask_delay_after_release
            and stationary_time >= ask_delay_after_release
        ):
            return "ASK"
        return "IDLE"
    ...
    if (
        ...
        and last_touched_time > cleanup_time_threshold
        and stationary_time > stationary_threshold
    ):
        return "CLEANUP_CANDIDATE"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sample",
        default='{"hand_distance":0.2,"user_present":1,"last_touched_time":5,"user_absent_time":0,"is_active_cup":0,"used_cup_candidate":1,"time_since_release":25.0,"stationary_time":4.0}',
