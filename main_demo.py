...
def draw_live_policy_overlay(frame, cups: list[dict], hand: dict, user_state: dict, predictions: list[dict]) -> np.ndarray:
    output = frame.copy()
    ...
    for cup in cups:
        ...
        cv2.circle(output, (cx, cy), 5, draw_color, -1)
        cv2.putText(output, f"Cup {cup['cup_id']} -> {action} ({confidence:.2f})", (x1, max(22, y1 - 32)), cv2.FONT_HERSHEY_SIMPLEX, 0.58, draw_color, 2)
        cv2.putText(
            output,
        ...
        cv2.putText(
            ...
            (x1, min(output.shape[0] - 28, y2 + 18)),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.42,
            draw_color,
            1,
        ...
        if cup.get("is_active_cup", 0):
            cv2.putText(output, "ACTIVE", (x1, min(output.shape[0] - 8, y2 + 38)), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (0, 255, 255), 2)
        if cup.get("used_cup_candidate", 0):
            cv2.putText(output, "USED", (x2 - 58, max(20, y1 - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.45, draw_color, 2)
        if action != raw_action:
            cv2.putText(output, f"raw={raw_action} -> {action}", (x1, min(output.shape[0] - 8, y2 + 56)), cv2.FONT_HERSHEY_SIMPLEX, 0.42, draw_color, 1)
        ...
        if action == "ASK" and not ask_message_drawn:
            cv2.putText(output, "이 잔 치워드릴까요?", (10, output.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.9, draw_color, 3)
